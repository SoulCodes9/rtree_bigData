...
def main():
    ...
    with open('10dp.txt', 'r') as file:
        for d in file.readlines():
            d = d.split()
            if len(d) == 2:
                data.append({'x': int(d[0]), 'y': int(d[1])})
            else:
                query.append({'x1': int(d[0]),
                              'x2': int(d[1]),
                              'y1': int(d[2]),
                              'y2': int(d[3])})
    print(data)
    ...
    print('our root node', rtree.root)
    for p in data:
        rtree.insert(rtree.root, p)
    print('Complete, the time taken was', (time.time()) - start)
    print('The MBR of this tree is:', rtree.root.mbr)

    result = []
    for q in query:
        result.append(rtree.RQ(rtree.root, q))
    print(result)
    ...
    def overflow(self):  # Check if Leaf has reached maximum capacity
        if self.leaf() == True:  # If node is a leaf
            if len(self.data_points) > B:  # B is the maximum capacity of the data points
                print('Data points are overflown for node', self)
                return True
            else:
                return False
        else:
            if len(self.child_node) > B:
                print('Children nodes are overflown')
                return True
            else:
                return False
...
class RTree(object):
    ...
    #     count = 0
    #     if u.leaf():
    #         print('Checking the leaf nodes within node', u)
    #         for p in u.data_points:
    #             if self.is_covered(p, q):
    #                 print('This point', p, 'is within the range!')
    #                 count = count + 1
    #     else:
    #         print('Checking the children nodes')
    #         for c in u.child_node:
    #             if self.overlap(c.mbr, q):
    #                 print('Node', c, 'overlaps with the query, traverse into it')
    #                 next = c
    #                 count += self.RQ(next, q)
    #     return count
    ...
    def insert(self, u, p):
        if u.leaf():
            ...
            print('insert into leaf node', u, 'Data points', p)
            if u.overflow():
                print('Leaf node', u, 'is overflown now we handle the overflow')
    ...
    def split(self, u):
        ...
        for ch in best_s2.child_node:
            ch.parent = best_s2
        print('The best splits are', best_s1, 'and \n', best_s2)
        return best_s1, best_s2

    # Adding the child into a node
    def add_child(self, node, child):
        print('Adding new child node:', child, 'into Node', node)
    ...
    def add_data_p(self, node, data_point):
        print('adding datapoints into leaf node:', node)
